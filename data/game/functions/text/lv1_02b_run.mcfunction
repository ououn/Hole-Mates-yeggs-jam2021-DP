...
#   每个显示出现时的音效
execute as @e[type=minecraft:area_effect_cloud,tag=line,scores={displayCount=-1}] if score @s displayID = .tmp1 temp at @a run playsound minecraft:block.note_block.bell master @a ~ ~ ~ 1 2.0 0
