#   高频。自定义内容：替换最后一个tag
execute as @e[type=minecraft:area_effect_cloud,tag=place_hold,tag=text_lv2_02] run function uin:tool/font_name/0
...
#   物理清除
#   execute as @e[type=minecraft:area_effect_cloud,tag=text_lv2_02] run function uin:tool/kill_same_displayid

#   重写播放
#   execute as @e[type=minecraft:area_effect_cloud,tag=text_lv2_02,tag=line] run scoreboard players operation @s displayCount = @s displayFontName
