...
#   每个显示出现时的音效
execute as @e[type=minecraft:area_effect_cloud,tag=line,scores={displayCount=-1}] if score @s displayID = .tmp1 temp run playsound minecraft:block.note_block.basedrum master @a ~ ~ ~ 1 2.0 0

#   物理清除
#   execute as @e[type=minecraft:area_effect_cloud,tag=text_lv1_02a] run function uin:tool/kill_same_displayid

#   重写播放
#   execute as @e[type=minecraft:area_effect_cloud,tag=text_lv1_02a,tag=line] run scoreboard players operation @s displayCount = @s displayFontName
